...
class Config(object):
	def __init__(self, args):
		...
		self.train_file_size = 1    #for human annotated training data
	...
	def from_list_to_tensor(self,item,train = True):
		...
		sen_matrix = np.zeros((node_num,node_num,graph.graph['max_sentence_num'],len(item['document'])))    
	...
	def load_train_data(self):
		...
		for i in range (self.train_file_size):
			...
			self.train_file.extend(temp_file)   #
			print (i)
		del temp_file
	...
	def load_test_data(self):
		...
		for i in range (self.test_file_size):
		    self.test_file.extend(pickle.load(open(os.path.join(self.data_path, prefix + '_' +str(i) + '.pkl'),'rb')))   #load pickle file (train data..)
		print("Finish reading")
