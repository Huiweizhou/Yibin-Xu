...
class GraphCNN_multihead_gate(nn.Module):
	def __init__(self, config):
		super(GraphCNN_multihead_gate, self).__init__()
		self.config = config
		...
		hidden_size = 128
		input_size = config.data_word_vec.shape[1]		#100
		if self.use_entity_type:                                                       
			input_size += config.entity_type_size		#entity_type_size = 20
		...
		self.layerNum = 2		
		self.headNum = 4
		self.get_weighted_adj_matrix = GATAttention (hidden_size,hidden_size)		#CAGGC
		self.get_adj_matrix =nn.ModuleList([MultiHeadAttention(self.headNum,hidden_size) for _ in range (self.config.graph_hop-1)])		#MAGGC
		...
		for layer in range (self.config.graph_hop):
			if layer == 0:
				self.graphcnn.append(GraphConvolution(self.layerNum,hidden_size,hidden_size))                     
			else:
		...
		self.linear_word_att =  nn.ModuleList([nn.Linear(hidden_size*2, hidden_size) for _ in range(self.config.graph_hop)])        
		self.linear_sentence_att =  nn.ModuleList([nn.Linear(hidden_size*2, hidden_size) for _ in range(self.config.graph_hop)])     

		if self.use_type_feat:
			self.dense_layer = nn.Linear(hidden_size*(config.graph_hop+1)+ config.dis_size+config.entity_type_size ,hidden_size)  
		else:
		...
		if self.use_distance:
			self.dis_embed = nn.Embedding(config.dis_num, config.dis_size)#, padding_idx=10)		#21*20
	...
	def forward(self, document,document_ner,document_pos,adj_matrix,sen_matrix,pos_matrix_h,pos_matrix_t,node_pos,node_type,node_relative_pos):
		...
		context_output = torch.tanh(self.linear_re(context_output))
		...
		for i in range(self.config.graph_hop):
			...
			context_word_att = self.linear_word_att[i](context_word_att)       
			#sentence_level_attention
			...
			context_sent_att = self.linear_sentence_att[i](context_sent_att)     
		...
		relation_before_softmax_01 = self.bili_layer_01 (entity_feature_h.contiguous(),entity_feature_t.contiguous()) + self.classification_layer_01(entity_feature)
